...
{
    [McpServerTool(Name = "run_tests"), Description("Runs 'tpx verify documents' (build + unit tests + contract lint). tpx does not support a test-name filter today, so 'filter' is accepted but only echoed back, not applied.")]
    ...
    {
        var (exitCode, output) = await ProcessRunner.RunAsync("tpx", "verify documents", RepoPaths.RepoRoot);
        ...
        return exitCode == 0
            ? $"tpx verify documents: PASSED\n{output}{note}"
            : $"tpx verify documents: FAILED (exit {exitCode})\n{output}{note}";
    }

    [McpServerTool(Name = "get_migrations_status"), Description("Lists EF Core migrations defined for the Documents module (dotnet-ef migrations list --no-connect -- doesn't require a live database).")]
    public static async Task<string> GetMigrationsStatus()
    {
        if (!File.Exists(RepoPaths.InfrastructureProject))
            return "TPXSoft.Documents.Infrastructure.csproj not found -- nothing to inspect.";
