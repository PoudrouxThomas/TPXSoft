...
1. Confirm the endpoint A needs already exists by calling B's MCP server (`get_openapi` or `describe_entity`). Fall back to reading `contracts/<B>.vN.yaml` directly only if B has no MCP server registered in `.mcp.json` yet. If the endpoint doesn't exist, that's a `new-endpoint` task on B first — do that, then come back here.
